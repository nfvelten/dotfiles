...
//
// The grid is a picker: arrow keys (or hjkl) move a day cursor, clicking a
// cell selects it directly, and the agenda below always reflects whichever
// day is selected. Selection crossing a month boundary carries the view
// with it. Events come from `khal` (whatever vdirsyncer syncs into
// ~/.local/share/khal/calendars/) via bin/calendar-events, re-fetched
// whenever the visible grid range changes.
//
...
Panel {
  ...
  // ---- Day selection + agenda. Starts on today each time the panel opens;
  //      arrow keys and clicks move it from there. `events` is keyed by the
  ...
  function selectDate(d) {
    root.selectedDate = d
    if (d.getFullYear() !== root.viewYear || d.getMonth() !== root.viewMonth) {
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // Everything above the agenda, grouped so its total height can be
          // measured and subtracted from the available popup height — that
          // is how the agenda list below knows exactly how much room it can
          // use without ever pushing the whole panel into a scroll.
          Column {
            id: fixedColumn
            width: parent.width
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      readonly property bool hasEvents: (root.events[modelData.key] || []).length > 0

                      width: root.cellWidth
                      height: root.cellHeight
                      radius: Style.cornerRadius
                      // Today is outlined, not filled: a lit-up block shouts
                      // over a grid this quiet. Selection gets an actual
                      // fill — it is the one state that is a choice rather
                      // than a fact about the calendar.
                      color: selected
                      ...
                      MouseArea {
                        ...
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: root.selectDate(new Date(modelData.year, modelData.month, modelData.day))
                      }
                    }
                  }
                }
          ...
          // ---- Agenda for whichever day is selected. Below everything
          //      else because it is the detail view, not part of the grid
          //      itself — same relationship the hero date has to the grid,
          //      just pointed at the selection instead of always today.
          PanelSeparator { id: agendaSeparator; foreground: root.contentForeground }
          ...
          Column {
            ...
            // left in the popup — availableCardHeight minus everything else
            // in the column (fixedColumn, the separator, this header) —
            ...
            Flickable {
              ...
              height: Math.min(maxHeight, agendaList.implicitHeight)
              contentWidth: width
